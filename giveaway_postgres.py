...
async def _channel_subscription_line_clickable_html(
    ...
    *,
    ...
    invite_export_cache: chat_id → URL приглашения на время сборки одного сообщения (дедуп строк).
    Сама ссылка берётся из createChatInviteLink и кэша процесса, не из повторного export.
    """
    t = (token or "").strip()
    if not t:
        return ""
    ...
    if t.lstrip("-").isdigit():
        ...
        if chat is not None:
            ...
            # Доп. инвайт без срока/лимита (create), не отзывает остальные ссылки чата.
            try:
                cid_key = int(chat.id)
                inv = await _bot_stable_invite_link_for_chat(
                    bot, cid_key, invite_export_cache=invite_export_cache
                )
                if inv:
                    code = ""
    ...
    if t.startswith("http://") or t.startswith("https://"):
        tl_http = t.lower()
        # Полный https://t.me/+… в missing часто приходит так — иначе подпись = URL и превью «ссылка истекла».
        if "t.me/+" in tl_http or "t.me/joinchat/" in tl_http:
            code = ""
            if "t.me/+" in t.lower():
                code = t.split("t.me/+", 1)[1].split("?", 1)[0].strip("/")
            elif "t.me/joinchat/" in t.lower():
                code = t.split("t.me/joinchat/", 1)[1].split("?", 1)[0].strip("/")
            deep = f"tg://join?invite={code}" if code else t
            safe = html.escape(deep, quote=True)
            name = await _invite_link_display_title(t)
            label = html.escape((name or "Приватный чат").strip(), quote=False)
            return f'• {ch_emoji} <a href="{safe}">{label}</a>'
...
# Доп. инвайт через createChatInviteLink (без срока/лимита) не отзывает другие ссылки.
# exportChatInviteLink при каждом вызове отзывает предыдущую основную — не используем как основной путь.
_STABLE_BOT_INVITE_BY_CHAT: dict[int, str] = {}


async def _bot_stable_invite_link_for_chat(
    bot: Bot,
    chat_id: int,
    *,
    invite_export_cache: Optional[dict[int, str]] = None,
) -> Optional[str]:
    """Одна стабильная ссылка на чат для участников: create (без expire/limit), кэш по chat_id."""
    k = int(chat_id)
    if invite_export_cache is not None and k in invite_export_cache:
        return invite_export_cache[k]
    if k in _STABLE_BOT_INVITE_BY_CHAT:
        url = _STABLE_BOT_INVITE_BY_CHAT[k]
        if invite_export_cache is not None:
            invite_export_cache[k] = url
        return url
    url: Optional[str] = None
    try:
        link = await bot.create_chat_invite_link(
            chat_id=k,
            name="AniGive розыгрыш",
            creates_join_request=False,
        )
        url = (link.invite_link or "").strip() if link else None
    except Exception as e:
        log.debug("create_chat_invite_link chat=%s: %s", k, e)
    if not url:
        try:
            url = (await bot.export_chat_invite_link(k) or "").strip() or None
        except Exception as e2:
            log.debug("export_chat_invite_link fallback chat=%s: %s", k, e2)
            return None
    _STABLE_BOT_INVITE_BY_CHAT[k] = url
    if invite_export_cache is not None:
        invite_export_cache[k] = url
    return url
...
def _channel_request_chat_rights() -> ChatAdministratorRights:
    """Права для request_chat (канал): user_* и bot_* должны совпадать (иначе USER_RIGHTS_MISSING).
    Без can_promote_members — иначе системный выбор чата доступен по сути только владельцу канала.
    Расширенное добавление бота по ссылке — по-прежнему в _DL_ADMIN_CH."""
...
def _group_request_chat_rights() -> ChatAdministratorRights:
    """То же для «Добавить группу»: минимум флагов, чтобы админы без «добавлять админов» могли выбрать чат."""
    return ChatAdministratorRights(
        is_anonymous=False,
        can_manage_chat=False,
        can_delete_messages=True,
...
def _reply_btn_add_channel() -> KeyboardButton:
    r = _channel_request_chat_rights()
    return KeyboardButton(
        text="Добавить канал",
        request_chat=KeyboardButtonRequestChat(
            request_id=REQ_CHAT_ADD_CHANNEL,
            chat_is_channel=True,
            user_administrator_rights=r,
            bot_administrator_rights=r,
        ),
        **_pe_icon(_PE_SAVED_CHANNEL),
    )
...
async def _giveaway_dm_status_text_and_kb(
    ...
    # Один кэш на сборку сообщения — дедуп строк; стабильный URL из create + кэша процесса.
...
def _winner_link_label_html(raw: str, user_id: int) -> str:
    """Текст внутри <a href=\"tg://user?id=…\">: без управляющих символов и с полным HTML-escape (в т.ч. кавычки)."""
    s = (raw or "").strip()
    s = "".join(ch for ch in s if ord(ch) >= 32 or ch in "\t")
    s = re.sub(r"\s+", " ", s).strip()
    if not s:
        s = f"Пользователь {user_id}"
    # quote=True — иначе " или & в имени ломают разметку и ссылка не строится (имя как обычный текст).
...
async def cb_lottery_pick(query: CallbackQuery, bot: Bot) -> None:
    ...
    creator_id = int(g.get("created_by") or 0)
    if creator_id > 0:
        winner_name = _winner_link_label_html(
